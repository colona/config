...
" Last Change:  2003 Sep 16 - Added 'private' as keyword (JY)
...
function GetOCamlIndent()
  ...
  elseif line =~ '^\s*|'
    if lline !~ '^\s*\(|\|\(match\|type\|with\)\>\)\|\<\(function\|parser\|private\|with\)\s*$'
      call search('|', 'bW')
      return indent(searchpair('^\s*\(match\|type\)\>\|\<\(function\|parser\|private\|with\)\s*$', '', '|', 'bWn', 'synIDattr(synID(line("."), col("."), 0), "name") =~? "string\\|comment" || getline(".") =~ "\\[|\\||\\]" && getline(".") !~ "^\\s*|.*->"'))
    else return ind
    endif

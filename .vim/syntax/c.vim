...
if exists("c_epita")
  if exists("cxx_epita")
    ...
    syn match	cEnum		display "^enum\t\+e_[a-z][a-z0-9_]*"
    syn match	cEnumError	display "^enum \+[^e][^_][a-z][a-z0-9_]*"
    syn match	cStruct		display "^struct\t\+s_[a-z][a-z0-9_]*"
    syn match	cStructError	display "^struct \+[^s][^_][a-z][a-z0-9_]*"
    syn match	cUnion		display "^union\t\+u_[a-z][a-z0-9_]*"
    syn match	cUnionError	display "^union \+[^u][^_][a-z][a-z0-9_]*"
    syn match	cEnum		display "enum e_[a-z][a-z0-9_]*"
    syn match	cStruct		display "struct s_[a-z][a-z0-9_]*"
    syn match	cUnion		display "union u_[a-z][a-z0-9_]*"
    syn match 	cFct		display "\w\+([^)]"me=e-2
  endif
  syn match 	cFctError	display "^\w\+\(\s\w\+\)*\s* \s*\**\w\+\s*("me=e-1
endif
